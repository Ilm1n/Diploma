...
<template>
  ...
  >
    <div class="flex flex-col gap-6 pt-2">
      <template v-if="!invitationLink">
        ...
        <div class="flex flex-col gap-2">
          ...
          <p class="text-[10px] text-slate-400">
            Только этот пользователь сможет принять приглашение
          </p>
        </div>
      ...
      <template v-else>
        <div class="flex flex-col gap-4 py-4 text-center animate-fade-in">
          <div
            class="w-16 h-16 bg-green-50 dark:bg-green-900/20 text-green-500 rounded-full flex items-center justify-center mx-auto mb-2"
          >
            <i class="pi pi-check text-2xl"></i>
          </div>
          <h3 class="font-bold text-xl">Ссылка готова!</h3>
          <p class="text-sm text-slate-500">
            Отправьте её коллеге. Она будет активна
            {{ expireOptions.find((o) => o.value === expiresInDays)?.label }}.
          </p>
          ...
          <div class="flex gap-2 mt-4">
            <InputText
              ...
              readonly
              class="flex-1 !bg-slate-50 dark:!bg-dark-bg"
            />
          ...
          <Button
            label="Создать другое приглашение"
            text
            class="!mt-4 !text-xs"
    ...
    <InviteQrDialog v-model:visible="isQrVisible" :value="invitationLink" />
  </Dialog>
</template>
