...
const onFileSelect = async (event: Event) => {
  ...
  try {
    ...
    toast.add({ severity: 'success', summary: 'Успешно', detail: 'Аватар обновлен', life: 3000 });
  } catch (e) {
    toast.add({ severity: 'error', summary: 'Ошибка', detail: 'Не удалось загрузить фото', life: 3000 });
  } finally {
...
const onSubmit = handleSubmit(async (values) => {
  try {
    ...
    toast.add({ severity: 'success', summary: 'Сохранено', detail: 'Данные профиля обновлены', life: 3000 });
  } catch (e) {
...
<!--    <ConfirmDialog />-->
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-8">
      ...
      <div class="md:col-span-1">
        <div class="bg-white dark:bg-dark-surface p-6 rounded-2xl border border-gray-200 dark:border-dark-border shadow-sm flex flex-col items-center text-center">
          ...
          <div class="relative group cursor-pointer mb-4" @click="triggerFileUpload">
            ...
            <!-- Overlay on Hover -->
            <div class="absolute inset-0 bg-black/50 rounded-full flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity duration-200">
          ...
          <p class="text-sm text-slate-500 dark:text-slate-400 mb-4">{{ authStore.user?.email }}</p>
      ...
      <div class="md:col-span-2">
        <div class="bg-white dark:bg-dark-surface p-6 rounded-2xl border border-gray-200 dark:border-dark-border shadow-sm">
          <h2 class="text-lg font-bold text-slate-800 dark:text-white mb-6 border-b border-gray-100 dark:border-slate-700 pb-2">
            Личные данные
          </h2>
          ...
          <form @submit.prevent="onSubmit" class="space-y-6">
            ...
            <div class="flex flex-col gap-2">
              <label for="username" class="font-medium text-slate-700 dark:text-slate-300">Никнейм</label>
              <InputText
                  id="username"
                  v-model="username"
            ...
            <!-- Full Name -->
            <div class="flex flex-col gap-2">
              <label for="fullname" class="font-medium text-slate-700 dark:text-slate-300">Полное имя</label>
              <InputText
                  id="fullname"
                  v-model="fullName"
