...
watch(localTagIds, (newVal) => {
  ...
  const isSame = newVal.length === currentIds.length && newVal.every(id => currentIds.includes(id));
  ...
  >
      ...
      <div v-else class="flex flex-col gap-8 pb-20">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div class="lg:col-span-2 flex flex-col gap-6">
            ...
            <!-- Даты -->
            <div class="flex flex-col gap-1 mb-4 p-3 bg-gray-50 dark:bg-dark-bg/30 rounded-lg border border-gray-100 dark:border-dark-border">
              <div class="flex justify-between text-[10px] text-slate-500">
                <span>Создано:</span>
                <span class="font-medium text-slate-700 dark:text-slate-300">{{ formatDate(store.selectedTask?.createdAt) }}</span>
              </div>
              <div class="flex justify-between text-[10px] text-slate-500">
                <span>Обновлено:</span>
                <span class="font-medium text-slate-700 dark:text-slate-300">{{ formatDate(store.selectedTask?.updatedAt) }}</span>
              </div>
            </div>
          </div>
    ...
    <template #footer>
      <div class="flex items-center justify-between w-full pt-2">
        <!-- Status Indicator -->
        <div class="flex items-center gap-2 text-xs transition-all duration-300 h-8">
          <template v-if="isSaving">
            <i class="pi pi-spin pi-spinner text-primary-500"></i>
            <span class="text-slate-500 dark:text-slate-400">Сохранение...</span>
          </template>
          <template v-else-if="lastSavedAt">
            <i class="pi pi-check text-green-500"></i>
            <span class="text-slate-500 dark:text-slate-400">
            Изменения сохранены <span class="text-slate-400 text-[10px]">({{ lastSavedTime }})</span>
          </span>
          </template>
        </div>
